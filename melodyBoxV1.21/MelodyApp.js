class MelodyApp {
    constructor() {
        ...
        //変数
        this.bpmTime   = 1000 / (this.sto.bpm/60);
        this.vol       = 0.05;
    ...
    bindUI(){
        ...
        this.$key.addEventListener("input", ()=> {
            this.sto.key = parseInt(this.$key.value, 10);
            console.log(this.sto);
        ...
        this.$bpm.addEventListener("input", ()=> {
            this.sto.bpm = parseInt(this.$bpm.value, 10);
            this.bpmTime   = 1000 / (this.sto.bpm/60);
    ...
    step(i){
        ...
        for(let bit=0; bit<this.scale; bit++){
            ...
            if(
                ...
                //音を鳴らす・音の長さの調節
                const Hz = 440*2**((bit-9+this.sto.key)/12);
